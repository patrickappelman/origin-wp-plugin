...
// Custom sanitization for Job_Description to allow specific HTML tags without inline styles and replace non-breaking spaces
function zoho_recruit_sanitize_job_description( $content ) {
    // Decode HTML entities to handle encoded characters
    $content = html_entity_decode( $content, ENT_QUOTES | ENT_HTML5, 'UTF-8' );
    
    // Replace non-breaking spaces (U+00A0, &nbsp;) with regular spaces
    $content = str_replace( [ "\xc2\xa0", "&nbsp;" ], ' ', $content );
    
    // Log input before sanitization
...
function zoho_recruit_test_sanitization() {
    if ( current_user_can( 'manage_options' ) && isset( $_GET['zoho_test_sanitization'] ) ) {
        // Include non-breaking spaces in test HTML
        ...
        $sanitized = zoho_recruit_sanitize_job_description( $test_html );
...
add_action( 'admin_init', 'zoho_recruit_test_sanitization' );
...
add_action( 'zoho_recruit_sync_jobs_cron', 'zoho_recruit_sync_jobs' );
register_activation_hook( __FILE__, function() {
    ...
    if ( !wp_next_scheduled( 'zoho_recruit_sync_jobs_cron' ) ) {
        wp_schedule_event( time(), 'hourly', 'zoho_recruit_sync_jobs_cron' );
    }
});
register_deactivation_hook( __FILE__, function() {
    wp_clear_scheduled_hook( 'zoho_recruit_refresh_token_cron' );
    wp_clear_scheduled_hook( 'zoho_recruit_sync_jobs_cron' );
});
...
function zoho_recruit_settings_page() {
    ...
    if ( isset( $_POST['zoho_recruit_sync_jobs'] ) && check_admin_referer( 'zoho_recruit_sync_jobs_nonce' ) ) {
        $sync_result = zoho_recruit_sync_jobs();
    }
...
function zoho_recruit_get_access_token() {
    $access_token = get_option( 'zoho_recruit_access_token', '' );
    $token_expires = get_option( 'zoho_recruit_token_expires', 0 );
...
function zoho_recruit_test_api() {
    $access_token = zoho_recruit_get_access_token();
...
// Fetch job details by ID to get HTML Job_Description
function zoho_recruit_get_job_by_id( $zoho_id, $access_token ) {
    $api_base_url = 'https://recruit.zoho.eu/recruit/v2/';
    ...
    if ( $response_code !== 200 ) {
        $error_message = isset( $body['message'] ) ? $body['message'] : 'HTTP ' . $response_code;
...
function zoho_recruit_sync_jobs() {
    ...
    // Log post_content before saving and list filters
    ...
    foreach ( $body['data'] as $job ) {
        ...
        // Use Job_Opening_ID if available; otherwise, use id as fallback for matching
        ...
        // Parse Modified_Time
        $modified_time = isset( $job['Modified_Time'] ) ? $job['Modified_Time'] : '';
        $zoho_modified = '';
        if ( $modified_time ) {
            try {
        ...
        // If no update needed or new job, proceed with processing
        if ( $needs_update || empty( $existing_posts ) ) {
            // Fetch HTML Job_Description
            $job_details = zoho_recruit_get_job_by_id( $zoho_id, $access_token );
            if ( is_wp_error( $job_details ) || !isset( $job_details['Job_Description'] ) ) {
                ...
                continue; // Skip this job if HTML fetch fails
            }
            ...
            $sanitized_description = zoho_recruit_sanitize_job_description( $job_description );
            ...
            // Store raw and sanitized description in meta
            ...
            foreach ( $acf_mappings as $acf_name => $mapping ) {
                $value = isset( $job[$mapping['field']] ) ? $job[$mapping['field']] : '';
                if ( $value !== '' ) {
    ...
    // Remove post_content logging filter
    remove_filter( 'wp_insert_post_data', $log_post_content, 9 );

    // Delete posts for non-existent jobs
    $args = [
        'post_type' => 'job',
        'posts_per_page' => -1,
        'post_status' => 'any',
